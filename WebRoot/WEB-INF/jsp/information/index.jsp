     ...
     <div class="swiper-container" id="topNav">
          <ul class="swiper-wrapper">
			<c:forEach items="${daohangMKlist }" var="daohangMKlists" varStatus="vs" >
	                <a id="tid${vs.index+1}" ttid="${daohangMKlists.goods_mokuai_id}" onclick="yilan('${daohangMKlists.goods_mokuai_id}',this)" href="javascript:void(0)" class="swiper-slide"><img src="${daohangMKlists.goods_mokuai_Img_url }" alt=""/><li style="margin-left:0;">${daohangMKlists.title }</li></a>
			</c:forEach>
          </ul>
    </div>
...
//by yym 数组集合操作+
  ...
  <script type="text/javascript">
  		function xuanran(mkid){
  			$(".bgf").empty();
  			$("#food_sort").empty();
  		 	//清空
              /*$("div.bgf *").each(function() { // "*"表示div.gouwuche下的所有元素
          		  $(this).remove();
       		 });*/
  			var appendhtml = '';
  			$.post('api/kehu/getDataByMKID.do',{goods_mokuai_id:mkid},function(result){
  			  //left html
  			   var lefthtm = '';
  			   for(var t=0;t<result.goodsCategoryList.length;t++){
  					lefthtm+='<li id="sts'+(t+1)+'" class="foodsort"><a data-tpye="#st'+(t+1)+'">'+result.goodsCategoryList[t].categoryName+'</a></li>';
  			   }
  			   $("#food_sort").append(lefthtm);
  			   //初始化左侧菜单
  			   leftMenuinit();
  			   initLeftMenuEvent();
  				...
  				for(var i=0;i<result.resultlist.length;i++){
		  			appendhtml+='<h6 class="foodlist_label" id="st'+(i+1)+'" style="text-align: left;">'+result.resultlist[i][0].categoryName+'</h6>';
		  			appendhtml+='<div class="foodlist_box">';
		  			//二级循环
		  			for(var j=0;j<result.resultlist[i].length;j++){
		  			//alert(result.resultlist[i][j].goodsName)
			  			appendhtml+='<ul>';
			  			appendhtml+='<li>';
			  			appendhtml+='<div class="img_list_box" tid="'+result.resultlist[i][j].goods_id+'">';
			  			appendhtml+='<img  class="img_list_img" src="'+result.resultlist[i][j].goodsImg+'"></div>';
			  			appendhtml+='<div class="shop_style">';
			  			appendhtml+='<h3 class="food_img_name">'+result.resultlist[i][j].goodsName+'</h3>';
			  			appendhtml+='<p class="font12 clo9" style="margin-top:5px;">最高￥'+result.resultlist[i][j].topPrice+'</p>';
			  			appendhtml+='<p class="font12 clo9">建议￥'+result.resultlist[i][j].advisePrice+'</p>';
			  			appendhtml+='<div class="jiacar"><div class="redprice">';
			  			appendhtml+='<span>￥</span><span class="price">'+result.resultlist[i][j].shangjiaPrice+'/'+result.resultlist[i][j].danwei+'</span></div>';
			  			appendhtml+='<div class="weui-flex__item shop-cart">';
			  			appendhtml+='<div class="d-stock">';
			  			appendhtml+='<a class="increase" onclick="addCar(\''+result.resultlist[i][j].goods_id+'\',\''+result.resultlist[i][j].goodsName+'\',\''+result.resultlist[i][j].shangjiaPrice+'\',\''+result.resultlist[i][j].danwei+'\',\''+result.resultlist[i][j].goodsImg+'\',this)">+</a>';
			  			appendhtml+='<input id="num" nid="n'+(j+1)+'" class="result text_box" mid="'+result.resultlist[i][j].goods_id+'" mstr="'+result.resultlist[i][j].goods_id+','+result.resultlist[i][j].shangjiaPrice+','+result.resultlist[i][j].danwei+','+result.resultlist[i][j].goodsImg+'" type="text" value="0">';
			  			appendhtml+='<a class="minus decrease" onclick="reduce(\''+result.resultlist[i][j].goods_id+'\',\''+result.resultlist[i][j].goodsName+'\',\''+result.resultlist[i][j].shangjiaPrice+'\',\''+result.resultlist[i][j].danwei+'\',\''+result.resultlist[i][j].goodsImg+'\',this)">-</a>';
			  			appendhtml+='</div>';
			  			appendhtml+='</div>';
			  			appendhtml+='</div>';
			  			appendhtml+='</div>';
...
//从确认订单页面返回 刷新当前页
        ...
        $(function(){
			...
			//if(tid=="" || tid==undefined){
				$("#tid1").addClass("hot");
				var mkid=$("#tid1").attr("ttid");
				//js xuanran
				//$(".bgf").empty();
  				//$("#food_sort").empty();
				xuanran(mkid);
				//location.href="api/kehu/index.do?goods_mokuai_id="+mkid;
			//}else{
			//	$("#"+tid).addClass("hot");
			//}
         ...
         var price=0;
           /* $(".shop-cart").each(function(){
   			...
   			//从map中计算合计
   			 ...
   			 var strs = map.arr[i].value.split(",");
	   			 for(var j=0;j<1;j++){
	   			 	console.log(strs[1]*strs[3])
	   			 	price+=parseInt(strs[1])*parseInt(strs[3]);
	   			 }
            }
...
function initLeftMenuEvent(){
    ...
    $("#food_sort>li").click(function(){
        $('#scroll_food').scrollTop(arr[$(this).index()]-15);
  ...
  <script type="text/javascript">
        $(function(){
        	...
        	if(LocationCity!="" && LocationCity!=undefined){
        		console.log("--存在session位置--");
        		//把session中的位置设置进去
        		$(".dizhi").text(LocationCity); 
        	}else{
				onloadss();            
        	}
        });
			    ...
			    BMap.Convertor.translate(gpsPoint, 0, function (point){  
				    ...
				    geoc.getLocation(point, function (rs){ 
				   	 console.log(rs); 
				         ...
				         //设置session
						$.post('api/kehu/setLocationToSession.do',{city:addComp.city},function(data){
							console.log(data)
						});
