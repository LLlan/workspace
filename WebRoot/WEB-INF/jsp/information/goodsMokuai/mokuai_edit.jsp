	...
	<form action="api/kehu/${msg }.do" name="Form" id="Form" method="post" enctype="multipart/form-data">
		...
		<table id="table_report"  class="table table-striped table-bordered table-hover">
			<tr>
				<td>模块图片:</td>
				<td>
					<c:if test="${pd == null || pd.goods_mokuai_Img_url == '' || pd.goods_mokuai_Img_url == null }">
						<input type="" id="file" name="file" onchange="fileType(this)" onclick="jump('${pd.goods_mokuai_id}');" />
					</c:if>
					<c:if test="${pd != null && pd.goods_mokuai_Img_url != '' && pd.goods_mokuai_Img_url != null }">
						<a href="javascript:void(0);" onclick="jump('${pd.goods_mokuai_id}');" target=""><img src="<%=basePath%>${pd.goods_mokuai_Img_url}" width="210"/></a>
						<input type="hidden" name="file"  value="${pd.goods_mokuai_Img_url }"/>
						<input type="" id="file" onchange="fileType(this)" onclick="jump('${pd.goods_mokuai_id}');" value="${pd.goods_mokuai_Img_url}"/>
					</c:if>
				</td>
			</tr>
			<tr>
