	...
	<input type="hidden" name="goods_id" value="${pd.goods_id}"/>
		...
		<table id="table_report"  class="table table-striped table-bordered table-hover">
			<tr>
				<td>商品名称:</td>
				<td>
					<input type="text" name="goodsName" id="goodsName" value="${goodsName }" placeholder="商品名称"/>
				</td>
			</tr>
			<tr>
				<td>商品分类:</td>
				<td>
					<select name="goods_category_id" id="goods_category_id">
						...
						<c:forEach items="${list }" var="list">
							<option value="${list.goods_category_id }" <c:if test="${list.goods_category_id==goods_category_fid }">selected="selected"</c:if>>${list.categoryName }</option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td> 
					...
					<c:if test="${filePath == null || filePath == '' || empty filePath}">
						<input type="" id="goodsImg" name="goodsImg" onchange="fileType(this)" onclick="jump();" />
					</c:if>
					<c:if test="${filePath != null && filePath != '' && filePath != null }">
						...
						<input type="hidden" name="goodsImg" value="${filePath }"/>
						<input type="" id="goodsImg" onchange="fileType(this)" onclick="jump();" value="${filePath}"/>
					</c:if>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="goodsState" id="goodsState" title="是否">
						<option value="1" <c:if test="${goodsState == '1' }">selected</c:if> >上架</option>
						<option value="0" <c:if test="${goodsState == '0' }">selected</c:if> >下架</option>
					</select>
				</td>
			</tr>
			<tr>
				<td>商品详情简介:</td>
				<td>
					<textarea rows="" cols="" name="goodsIntroduce" id="goodsIntroduce">${goodsIntroduce }</textarea>
				</td>
			</tr>
			<tr>
				<td>商品最高价:</td>
				<td>
					<input type="text" name="topPrice" id="topPrice" value="${topPrice }" placeholder="商品最高价"/>
				</td>
			</tr>
			<tr>
				<td>商品建议价:</td>
				<td>
					<input type="text" name="advisePrice" id="advisePrice" value="${advisePrice }" placeholder="商品建议价"/>
				</td>
			</tr>
			<tr>
				<td>商品上架价:</td>
				<td>
					<input type="text" name="shangjiaPrice" id="shangjiaPrice" value="${shangjiaPrice }" placeholder="商品上架价"/>
				</td>
			</tr>
			<tr>
				<td>商户价:</td>
				<td>
					<input type="text" name="shanghuPrice" id="shanghuPrice" value="${shanghuPrice }" placeholder="商户价"/>
				</td>
			</tr>
			<%-- <tr>
				<td>商品起订描述:</td>
			...
			<td>
				<select name="danwei_id" id="danwei_id">
					...
					<c:forEach items="${sysManagerList }" var="sysManagerLists">
						<option value="${sysManagerLists.danwei_id }" <c:if test="${sysManagerLists.danwei_id==danwei_id }">selected="selected"</c:if>>${sysManagerLists.name }</option>
					</c:forEach>
				</select>
			</td>
			</tr>
			<tr>
				<td>商品库存:</td>
				<td>
					<input type="text" name="Stock" id="Stock" value="${Stock }" placeholder="商品库存"/>
				</td>
			</tr>
			<%-- <tr>
				<td>商品规格:</td>
...
<script type="text/javascript">
	...
	function jump(){
		...
		var Stock = $("#Stock").val();
		window.location.href="api/kehu/jump?goodsName="+goodsName+"&goods_category_id="
