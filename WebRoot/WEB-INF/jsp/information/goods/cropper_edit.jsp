...
<br><br>
  <!-- Content -->
<form action="<%=basePath%>api/kehu/ceshiupload.do?goods_id=${pd.goods_id}&goodsName=${pd.goodsName }&goods_category_fid=${pd.goods_category_fid }&goodsState=${pd.goodsState}&topPrice=${pd.topPrice}&advisePrice=${pd.advisePrice}&shangjiaPrice=${pd.shangjiaPrice}&shanghuPrice=${pd.shanghuPrice}&goodsIntroduce=${pd.goodsIntroduce}&Stock=${pd.Stock}&danwei_id=${pd.danwei_id}" method="post" enctype="multipart/form-data" id="myForm">
  <div class="container">
    <div class="row">
      <div class="col-md-9">
        <!-- <h3>Demo:</h3> -->
		<h4 style="text-align:left;">图片裁剪</h4>
        <div class="img-container">
         <c:if test="${pds == null || pds.goodsImg == '' || pds.goodsImg == null }">
         	<img id="image" src="">
         	<input type="hidden" name="goodsImgs" id="goodsImgs" value="${pds.goodsImg }"/>
		</c:if>
		<c:if test="${pds != null && pds.goodsImg != '' && pds.goodsImg != null }">
	        <img id="image" src="<%=basePath%>${pds.goodsImg}">
			<input type="hidden" name="goodsImg" id="goodsImg" value="${pds.goodsImg }"/>
			<input type="hidden" name="goods_id" id="goods_id" value="${pds.goods_id }"/>
		</c:if>
        </div>
      </div>
      <div class="col-md-3">
    ...
    <div class="row">
      <div class="col-md-9 docs-buttons">
        ...
        <div class="btn-group">
        	<button type="button" class="btn btn-primary dianjia" data-method="destroy" title="Destroy" style="width: 60px;">
            <span class="docs-tooltip" data-toggle="tooltip" data-animation="false" title="保存裁剪图片">
              <span>保存</span>
            </span>
          </button>
          <button onclick="quxiao();" type="button" class="btn btn-primary" data-method="destroy" title="Destroy" style="width: 60px;">
	           <span class="docs-tooltip" data-toggle="tooltip" data-animation="false" title="取消裁剪图片">
	            <span>返回</span>
	           </span>
          </button>
        </div>
