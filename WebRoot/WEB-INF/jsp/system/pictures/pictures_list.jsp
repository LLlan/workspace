...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="pictures/picturesList.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td>
									       <c:if test="${var.STATUS == '0' }"><span class="label label-important arrowed-in">已停用</span></c:if>
									       <c:if test="${var.STATUS == '1' }"><span class="label label-success arrowed">使用中</span></c:if>
								        </td>
										<td>${var.BZ}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id,PATH){
				bootbox.confirm("确定删除该条数据?", function(result){
					if(result){
						top.jzts();
