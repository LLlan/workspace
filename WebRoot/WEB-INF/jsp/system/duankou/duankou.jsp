...
<body>
	<form action="api/kehu/${msg }.do?user_kehu_id=${pd.user_kehu_id}" id="residenceForm" method="post">
	     <input type="hidden" name="user_kehu_id" id="user_kehu_id" value="${pd.user_kehu_id}">
		<div id="zhongxin">
		<table class="table table-striped table-bordered">
			<tr>
				<td width="110px;">端口名称：</td>
				<td>
				 <select class="chzn-select" name="duankou_id" data-placeholder="请选择分配端口" style="vertical-align:top;">
				           <option value=""></option>
				           <c:forEach items="${duankoulist}" var="duankou">
					          <option value="${duankou.duankou_id}" <c:if test="${duankou.duankou_id eq pd.duankou_fid }">selected</c:if>>${duankou.dkmc}</option>
				            </c:forEach> 
				 </select>
				</td>
			</tr>
		...
		function abc(){
			$("#duankou_id1").val($("#duankou_id").val());
			alert($("#duankou_id").val())
		}
		//货物单价输入框监听事件，计算金额
