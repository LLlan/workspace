...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="api/duankou/duankoulistPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
		...
		<tbody>
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty duankouList}">
						<c:forEach items="${duankouList}" var="rlist" varStatus="vs">
							...
							<%-- 	<td class='center' style="width: 30px;">
								...
								<td>${rlist.dkmc}</td>
								<td>
								<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${rlist.duankou_id }')" ><i class='icon-edit'></i></a>
								<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.duankou_id }')"><i class='icon-trash'></i></a>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>api/duankou/goAddPage.do';
			 diag.Width = 575;
			 diag.Height = 485;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function edit(tagID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>api/duankou/edit.do?tagID='+tagID;
			 diag.Width = 575;
			 diag.Height = 485;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(tagID){
			 bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>api/duankou/del.do?tagID='+tagID;
