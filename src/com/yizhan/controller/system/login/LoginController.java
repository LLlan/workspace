package com.yizhan.controller.system.login;
...
public class LoginController extends BaseController {
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					...
					if(pd != null){
						...
						user.setESTATE_COMPANY_ID(pd.getString("ESTATE_COMPANY_ID"));
		...
		try{
			...
			if (user != null) {
				...
				if(null == session.getAttribute(Const.SESSION_allmenuList)){
					...
					if(Tools.notEmpty(roleRights)){
						for(Menu menu : allmenuList){
							...
							//如果是系统管理员，不筛选菜单，否则按指定用户去显示指定的菜单
							if(user.getUSERNAME().equals("admin")){
								System.out.println("======我是admin====");
								
							}else if(user.getUSERNAME().equals("lfd")){
								List<Menu> allmenu2 = new ArrayList<Menu>();
								Menu menu2 = new Menu();
								if(menu.getMENU_NAME().equals("商户管理")){
									menu2.setMENU_NAME(menu.getMENU_NAME());
									menu2.setSubMenu(menu.getSubMenu());
									allmenu2.add(menu2);
									allmenuList = allmenu2;
								}
				...
				if(null == session.getAttribute(Const.SESSION_QX)){
					session.setAttribute(Const.SESSION_QX, this.getUQX(session));	//按钮权限放到session中,这个是权限是要通过拦截器判断的
				}
	...
	public ModelAndView defaultPage(Page page) throws Exception{
		...
		pd.put("estate_company_id", user.getESTATE_COMPANY_ID());
		page.setPd(pd);
		
	/*	//小区动态
		List<PageData>	dynamicList = dynamicService.dynamicListPage(page);
		mv.addObject("dynamicList", dynamicList);
		//设备报修
		List<PageData> repairsList =  equipmentService.repairsListPage(page);
		mv.addObject("repairsList", repairsList);
		//小区列表
		List<PageData> residenceList  = residenceService.residenceListPage(page);
	...
	public Map<String, String> getUQX(Session session){
		...
		try {
			...
			map.put("ROLE_NAME", pd.getString("ROLE_NAME"));
